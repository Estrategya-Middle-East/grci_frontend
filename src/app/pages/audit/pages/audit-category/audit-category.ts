...
@Component({
  ...
  imports: [AuditCategoriesToolbar, AuditTable,NgbDropdownModule,RouterModule],
...
private auditService = inject(AuditItemService)
  ...
  deleteAuditItem(id:string,title:string) {
   ...
   EditAuditItem(id:string,name:string,describtion:string,storageLocationId:number) {
          ...
          ref.onClose.subscribe((result) => {
            if (result.value) {
              ...
              this.auditService.editAuditCategory({...result.value,id:id},id).subscribe({
                next:(res)=>{
